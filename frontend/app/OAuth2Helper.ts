...
export const refreshLogin: (tokenUri: string) => Promise<void> = (tokenUri) =>
  new Promise((resolve, reject) => {
    const refreshToken = localStorage.getItem("fibrecensus:refresh-token");
    if (!refreshToken) {
    ...
    const performRefresh = async () => {
      ...
      switch (response.status) {
        case 200:
          ...
          localStorage.setItem("fibrecensus:access-token", content.access_token);
          if (content.refresh_token)
            localStorage.setItem(
              "fibrecensus:refresh-token",
